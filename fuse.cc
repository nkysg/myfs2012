                 ...
                 struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret;
  yfs_client::inum inum = ino;
  ret = yfs->write(inum, buf, size, off);
  if (ret != yfs_client::OK) {
    fuse_reply_err(req, EIO);
    return;
  }
...
// invoked by fuseserver_lookup
yfs_client::status
look_up_by_name(yfs_client::inum parent, const char *name, bool &found,
    yfs_client::inum &inum, struct stat &st)
{
  yfs_client::status ret;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  ret = look_up_by_name(parent, name, found, inum, e.attr);
  if (ret != yfs_client::OK) {
    fuse_reply_err(req, ENOENT);
    return;
  }
